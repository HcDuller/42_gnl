...
/*   Updated: 2021/06/24 12:35:55 by hde-camp         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*my_line[256];
	char		*readings;
	int			r_size;

	if (shoul_it_run(fd, line, &my_line[fd], &readings))
	{
		while (!has_break(my_line[fd]))
		{
			r_size = read(fd, readings, BUFFER_SIZE);
			if (r_size <= 0)
				return (str_handler(line, &my_line[fd], r_size, &readings));
			my_line[fd] = str_append(my_line[fd], readings, 1);
		}
		*line = pre_break(my_line[fd]);
		my_line[fd] = pos_break(my_line[fd]);
		free(readings);
		return (1);
	}
	return (-1);
